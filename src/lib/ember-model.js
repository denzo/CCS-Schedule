  ...
  */
  ...
  _copyDirtyAttributesToData: function() {
    ...
    for (var i = 0, l = dirtyAttributes.length; i < l; i++) {
      ...
      // added by deniss.alimovs@gmail.com
      // start
      // ----------------------------
	  if (!Em.isEmpty(this.cacheFor(key))) {
	  // ----------------------------
	  // end
...
Ember.Model.reopenClass({
  ...
  _findFetchById: function(id, isFetch) {
	...
	console.log('Ember.Model.reopenClass._findFetchById', record.constructor, id);
  ...
  findFromCacheOrLoad: function(data) {
    ...
    // ------------------------------------------
    // start add by deniss.alimovs@gmail.com
    //
    // SlickGrid required id property
    //
    
    record.id = data[get(this, 'primaryKey')];
    
    // end add by deniss.alimovs@gmail.com
    // ------------------------------------------
  ...
  return Ember.computed(function(key, value) {
    ...
    if (arguments.length === 2) {
      ...
      if (dataValue !== value) {
        ...
        * added by deniss.alimovs@gmail.com
        */
        if (!dirtyAttributes.contains(key)) dirtyAttributes.pushObject(key);
      } else {
        dirtyAttributes.removeObject(key);
      }
